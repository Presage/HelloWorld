...
public class HelloWorldProtocol extends FSMProtocol {
	...
	enum States {
		START, HELLO_SENT, WORLD_RECEIVED, HELLO_RECEIVED
	};

	enum Transitions {
		SEND_HELLO, RECEIVE_WORLD, RECEIVE_HELLO
	};

	public HelloWorldProtocol(String agentName, NetworkAdaptor network) {
		super("HELLO WORLD", FSM.description(), network);
		this.name = agentName;
		logger = Logger.getLogger(HelloWorldProtocol.class.getName() + ", " + name);
		try {
			this.description
					...
					.addTransition(Transitions.SEND_HELLO,
							new EventTypeCondition(ConversationSpawnEvent.class), States.START,
							States.HELLO_SENT, new SpawnAction() {
								@Override
								public void processSpawn(ConversationSpawnEvent event,
									...
									conv.getNetwork().sendMessage(
											new UnicastMessage<Object>(Performative.PROPOSE,
													"HELLO", SimTime.get(), conv.getNetwork()
															.getAddress(), conv.recipients.get(0)));
								}
							})
					.addTransition(
							Transitions.RECEIVE_WORLD,
							new AndCondition(new MessageTypeCondition("WORLD"),
									new ConversationCondition()), States.HELLO_SENT,
							States.WORLD_RECEIVED, new MessageAction() {
								@Override
					...
					.addTransition(Transitions.RECEIVE_HELLO, new MessageTypeCondition("HELLO"),
							States.START, States.HELLO_RECEIVED,
							new InitialiseConversationAction() {
								...
								public void processInitialMessage(Message<?> message,
									...
									conv.getNetwork().sendMessage(
											new UnicastMessage<Object>(Performative.AGREE, "WORLD",
													SimTime.get(), conv.getNetwork().getAddress(),
													message.getFrom()));
								}
							});
